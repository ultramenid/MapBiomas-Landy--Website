<div>
    <x-cms.page-header title="Infographics" description="Infographic material published on the website.">
        <x-slot:actions>
            <x-cms.button href="{{ url('/cms/addinfographic') }}">New infographic</x-cms.button>
        </x-slot:actions>
    </x-cms.page-header>

    <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center">
        <input type="search" placeholder="Search infographics…"
        ...
        <x-cms.data-table>
            ...
            @foreach ($posts as $item)
                <tr class="transition-colors hover:bg-hover/50">
                    <x-cms.td>
                        <a href="{{ url('/cms/editinfographic/'.$item->id) }}" class="block max-w-xs">
                    ...
                    <x-cms.td class="text-right">
                        <x-cms.dropdown>
                            <x-cms.dropdown-item href="{{ url('/cms/editinfographic/'.$item->id) }}">Edit</x-cms.dropdown-item>
    ...
    @else
        <x-cms.empty-state
            title="{{ $query ? 'No matching infographics' : 'No infographics yet' }}"
            description="{{ $query ? 'Try a different search.' : 'Add the first infographic to see it here.' }}">
            @if (! $query)
                <x-slot:action>
                    <x-cms.button href="{{ url('/cms/addinfographic') }}">New infographic</x-cms.button>
                </x-slot:action>
    ...
    <x-cms.modal open="$wire.deleter" close="$wire.closeDelete()" title="Delete infographic">
