<div>
    <x-cms.page-header title="Edit news">
        <x-slot:actions>
            <x-cms.button variant="secondary" href="{{ url('/cms/listnews') }}">Cancel</x-cms.button>
    ...
    <div class="max-w-3xl space-y-4">
        ...
        <x-cms.panel title="Cover image">
            <x-cms.image-upload model="photo" hint="JPG or PNG. Upload a new image to replace the current one."
                                :preview-url="$photo?->temporaryUrl() ?? asset('/storage/files/photos/'.$uphoto)" />
        </x-cms.panel>
        ...
        <x-cms.form-tabs initial="id">
            <x-slot:en>
                <x-cms.panel title="English content">
                    <div class="space-y-4">
                        <x-cms.input name="titleEN" label="Title" maxlength="120" wire:model.defer="titleEN" placeholder="Title…" />
                        <x-cms.textarea name="descriptionEN" rows="4" label="Description" maxlength="255"
                                        wire:model.defer="descriptionEN" placeholder="Description…"></x-cms.textarea>
                        <div>
            ...
            <x-slot:idn>
                <x-cms.panel title="Konten Bahasa Indonesia">
                    <div class="space-y-4">
                        <x-cms.input name="titleID" label="Judul" maxlength="120" wire:model.defer="titleID" placeholder="Judul…" />
                        <x-cms.textarea name="descriptionID" rows="4" label="Deskripsi" maxlength="255"
                                        wire:model.defer="descriptionID" placeholder="Deskripsi…"></x-cms.textarea>
                        <div>
                            <span class="mb-1.5 block text-xs font-medium text-ink">Isi</span>
